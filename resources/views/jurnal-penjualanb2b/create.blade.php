                ...
                <form action="{{ route('jurnal-penjualanb2b.store', $type === 'pembayaran' ? $pembayaran->id : $pengiriman->id) }}" method="POST" id="form-jurnal">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-4">
                            ...
                            <input type="date" name="tanggal" class="form-control" value="{{ $type === 'pembayaran' ? \Carbon\Carbon::parse($pembayaran->tanggal_bayar)->format('Y-m-d') : \Carbon\Carbon::parse($pengiriman->tanggal_pengiriman)->format('Y-m-d') }}" required readonly style="background-color: #e9ecef; cursor: not-allowed;">
                        </div>
                        <div class="col-md-4">
