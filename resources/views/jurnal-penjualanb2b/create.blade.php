<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Input Jurnal Penjualan B2B</h2>
    </x-slot>

    <div class="container py-4">
        <div class="card shadow">
            <div class="card-header bg-primary text-white">
                <h4 class="mb-0">Input Jurnal Penjualan B2B</h4>
            </div>
            <div class="card-body">
                @if($errors->any())
                ...
                <form action="{{ route('jurnal-penjualanb2b.store', $pesanan->id) }}" method="POST" id="form-jurnal">
                    @csrf
                    <div class="row mb-4">
                        <div class="col-md-4">
                            <label class="form-label fw-bold">Tanggal</label>
                            <input type="date" name="tanggal" class="form-control"
                                value="{{ isset($pesanan) ? $pesanan->tanggal : date('Y-m-d') }}" required>
                        </div>

                        <div class="col-md-4">
                            <label class="form-label fw-bold">No. Referensi</label>
                            <input type="text" name="no_ref" class="form-control"
                                value="{{ isset($pesanan) ? 'REF-' . $pesanan->kode_pesanan : '' }}"
                        ...
                        <div class="col-md-4">
                            ...
                            <textarea name="deskripsi" class="form-control" rows="1" required
                                placeholder="Keterangan transaksi...">{{ isset($pesanan) ? 'Pencatatan jurnal atas penjualan B2B dengan No. Invoice: ' . $pesanan->kode_pesanan : '' }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between border-top pt-3">
                        <a href="{{ route('jurnal-penjualanb2b.index') }}" class="btn btn-light border">Batal</a>
