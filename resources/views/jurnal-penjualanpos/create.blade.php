<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Review & Input Jurnal Penjualan POS</h2>
    </x-slot>

    <div class="container py-4">
        <div class="card shadow">
            <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                <h4 class="mb-0">Form Konfirmasi Jurnal Omzet POS Harian</h4>
                <span class="badge bg-light text-dark fw-bold">Kode POS: {{ $penjualan->kode_transaksi }}</span>
            </div>
            <div class="card-body">
                @if($errors->any())
                ...
                <form action="{{ route('laporan.jurnal-penjualanpos.store', $penjualan->id) }}" method="POST" id="form-jurnal">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="no_ref" class="form-control" value="JV-POS-{{ $penjualan->kode_transaksi }}" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-bold">Deskripsi Jurnal</label>
                            <textarea name="deskripsi" class="form-control" rows="1" required>Pembukuan pendapatan kasir tunai outlet retail atas Kode Ringkasan: {{ $penjualan->kode_transaksi }}</textarea>
                        </div>
                    </div>
