<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Input Jurnal Penjualan POS</h2>
    </x-slot>

    <div class="container py-4">
        <div class="card shadow">
            <div class="card-header bg-primary text-white">
                <h4 class="mb-0">Input Jurnal Penjualan POS</h4>
            </div>
            <div class="card-body">
                @if($errors->any())
                ...
                <form action="{{ route('jurnal-penjualanpos.store', $penjualanpos->id) }}" method="POST" id="form-jurnal">
                    @csrf
                    <div class="row mb-4">
                        <div class="col-md-4">
                            <label class="form-label fw-bold">Tanggal</label>
                            <input type="date" name="tanggal" class="form-control"
                                value="{{ isset($penjualanpos) ? $penjualanpos->tanggal : date('Y-m-d') }}" required>
                        </div>

                        <div class="col-md-4">
                            <label class="form-label fw-bold">No. Referensi</label>
                            <input type="text" name="no_ref" class="form-control"
                                value="{{ isset($penjualanpos) ? 'REF-' . $penjualanpos->kode_penjualanpos : '' }}"
                        ...
                        <div class="col-md-4">
                            ...
                            <textarea name="deskripsi" class="form-control" rows="1" required
                                placeholder="Keterangan transaksi...">{{ isset($penjualanpos) ? 'Pencatatan jurnal atas penjualanpos dengan No. Invoice: ' . $penjualanpos->kode_penjualanpos : '' }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between border-top pt-3">
                        <a href="{{ route('jurnal-penjualanpos.index') }}" class="btn btn-light border">Batal</a>
